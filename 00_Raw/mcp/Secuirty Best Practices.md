...
APIs, creating  
"\[confused deputy\](https://en.wikipedia.org/wiki/Confused\_deputy\_problem)"  
...
sequenceDiagram  
    ...
    UA-\>\>TAS: Authorization request (client\_id: mcp-proxy)  
...
sequenceDiagram  
    ...
    A-\>\>M: Dynamically register malicious client, redirect\_uri: attacker.com  
    A-\>\>UA: Sends malicious link  
    UA-\>\>TAS: Authorization request (client\_id: mcp-proxy) \+ consent cookie  
...
sequenceDiagram  
    ...
    Client-\>\>MCP: Register with redirect\_uri  
    MCP--\>\>Client: client\_id

    Note over Client,ThirdParty: 2\. Authorization Request  
    Client-\>\>Browser: Open MCP server authorization URL  
    Browser-\>\>MCP: GET /authorize?client\_id=...\&redirect\_uri=...

    alt Check MCP Server Consent  
        MCP-\>\>MCP: Check consent for this client\_id  
    ...
    MCP-\>\>MCP: Store consent decision for client\_id

    Note over Client,ThirdParty: 3\. Forward to Third-Party  
    MCP-\>\>Browser: Redirect to third-party /authorize  
    Note over MCP: Use static client\_id for third-party

    Browser-\>\>ThirdParty: Authorization request (static client\_id)  
    ...
    MCP-\>\>ThirdParty: Exchange code for token (using static client\_id)  
    MCP-\>\>Browser: Redirect to client's registered redirect\_uri  
\`\`\`
...
sequenceDiagram  
    ...
    MCP--\>\>Client: 401 \+ resource\_metadata="http://169.254.169.254/..."
...
\* \[OWASP Top 10 A10:2021 \- SSRF\](https://owasp.org/Top10/2021/A10\_2021-Server-Side\_Request\_Forgery\_%28SSRF%29/):  
